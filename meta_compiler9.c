...
int get_sym_by_name(const char* name) {
...
int src = get_sym(); int dst = get_sym(); int tr = get_sym_by_name("__tr");
move_to(dst); emit_safe('7'); emit_safe('4'); emit_safe('8');
move_to(tr); emit_safe('7'); emit_safe('4'); emit_safe('8');
move_to(src); emit_safe('7');
move_to(dst); emit_safe('3'); move_to(tr); emit_safe('3'); move_to(src); emit_safe('4');
emit_safe('8');
move_to(tr); emit_safe('7');
move_to(src); emit_safe('3'); move_to(tr); emit_safe('4');
emit_safe('8');
}
else if(c == 'F') {
int addr = get_sym(); int val = get_num();
int t0 = get_sym_by_name("__t0"); int t1 = get_sym_by_name("__t1");
move_to(addr); emit_math(val, 'a', '4');
move_to(t0); emit_safe('7'); emit_safe('4'); emit_safe('8');
move_to(t1); emit_safe('7'); emit_safe('4'); emit_safe('8');
move_to(addr); emit_safe('7');
move_to(t0); emit_safe('3');
move_to(addr); emit_safe('7'); emit_safe('4'); emit_safe('8');
emit_safe('8');
move_to(t1); emit_safe('3');
move_to(t0); emit_safe('7');
move_to(t1); emit_safe('4');
move_to(t0); emit_safe('7'); emit_safe('4'); emit_safe('8');
emit_safe('8');
move_to(t1); emit_safe('7');
loop_stack[loop_sp++] = t1;
}
else if(c == 'U') {
if(loop_sp > 0) {
int t1 = loop_stack[--loop_sp];
move_to(t1); emit_safe('7'); emit_safe('4'); emit_safe('8'); emit_safe('8');
}
}
else if(c == '$') {
ungetc('$', stdin); int addr = get_sym(); move_to(addr);
}
if(out) emit(out);
}
if(count > 0) emit('x');
